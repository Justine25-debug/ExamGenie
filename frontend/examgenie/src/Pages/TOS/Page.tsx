...
const TOSPage = () => {
  ...
  const [exams, setExams] = useState<Record<number, string[]>>({});
  ...
  const generateExamFromTOS = (form: TOSForm): string[] => {
    ...
    if (!subjectName || !questionBank[subjectName]) return [];
    ...
    form.rowsData.forEach((row) => {
      ...
      const testType = row["testType"] as keyof typeof questionBank[string][string];

      if (
        itemAllocation > 0 &&
        questionBank[subjectName][topic] &&
        questionBank[subjectName][topic][testType]
      ) {
        ...
        selected.forEach((q) => questions.push(q.question));
      }
    });
  ...
  const handleInputChange = (
    ...
    updatedRows[rowIndex][column] = column === "testType" ? value : Number(value) || 0;
  ...
  const handleDelete = (formIndex: number) => {
    ...
    if (confirmDelete) {
      ...
      const deletedForm = updatedForms[formIndex];
      const listIndex = updatedList.findIndex(
        item => item.selectedSubject === deletedForm.selectedSubject && 
               item.selectedLevel === deletedForm.selectedLevel
      );
      if (listIndex !== -1) {
                        ...
                        <table className="table-auto w-full text-sm min-w-[1200px]">
                          ...
                          <tbody>
                            {learningOutcomes.map((topic, rowIndex) => {
                              ...
                              if (!form.rowsData[rowIndex]) {
                                form.rowsData[rowIndex] = {};
                              }
                              form.rowsData[rowIndex]["week"] = week;
                              form.rowsData[rowIndex]["topic"] = topicContent;
                              return (
                                <tr key={`row-${formIndex}-${rowIndex}`} className="border-t">
                                  ...
                                  <td className="p-2">
                                    <Input
                                      type="number"
                                      min={0}
                                      value={form.rowsData[rowIndex]?.["time"] ?? ""}
                                      onChange={(e) =>
                                  ...
                                  <td className="p-2 min-w-[200px]">
                                    <select
                                      value={form.rowsData[rowIndex]?.testType || "Multiple Choice"}
                                      onChange={(e) =>
            ...
            <DialogContent>
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto">
                {exams[fileUploadIndex].map((question, idx) => (
                  <div key={`question-${fileUploadIndex}-${idx}`} className="p-4 bg-background rounded border">
